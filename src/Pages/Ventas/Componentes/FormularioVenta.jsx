...
export const FormularioVenta = ({
    ...
    const showAlert = (message, variant = 'danger') => {
        ...
        setTimeout(() => {
            if (alertRef.current) alertRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }, 100);
    ...
    const handleGaranteChange = (e) => {
        const { name, value } = e.target;
        setFormData({ ...formData, garante: { ...formData.garante, [name]: value } });
    };
        ...
        if (nuevaNota.trim()) {
            setFormData({
                ...
                notas: [...formData.notas, {
                    texto: nuevaNota.trim(), fecha: new Date().toISOString(),
                    tipo: 'general', usuario: { nombre: vendedor || 'Vendedor' }
                }]
            });
            setNuevaNota('');
        }
    ...
    const agregarPago = () => {
        setFormData({ ...formData, pagos: [...formData.pagos, { monto: 0, metodo: 'efectivo', notas: [] }] });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.tipoVenta === 'plan_canje') {
            if (!formData.equipoCanje.nombre.trim()) { showAlert('El nombre del equipo a canjear es obligatorio', 'warning'); return; }
        ...
        <Card className="border-0 shadow-sm" style={{ borderRadius: '16px' }}>
            <Card.Body className="p-4 p-md-5">
                <div className="text-center mb-4">
                    <h4 className="fw-bold mb-1" style={{ color: '#1a1a1a' }}><i className="bi bi-cart-plus me-2" style={{ color: '#3483FA' }}></i>Nueva Venta</h4>
                    <p className="text-muted small mb-0">Completá los datos de la venta</p>
                    {clienteData && (
                        <div className="mt-2">
                            <Badge bg="success" className="me-1"><i className="bi bi-person-check me-1"></i>Cliente verificado</Badge>
                            <span className="text-muted small">{clienteData.nombreCompleto || clienteData.nombre + ' ' + clienteData.apellido} - DNI: {clienteData.dni}</span>
                        </div>
                    )}
                </div>

                {alert.show && (
                    <Alert ref={alertRef} variant={alert.variant} className="d-flex align-items-center rounded-3 border-0 shadow-sm mb-3"
                ...
                <Form onSubmit={handleSubmit}>
                            ...
                            <Col md={4}>
                                <Form.Group>
                                    ...
                                    <Form.Select name="tipoVenta" value={formData.tipoVenta} onChange={handleChange} className="rounded-3" disabled={isLoading}>
                                        {tiposVenta.map(tipo => <option key={tipo.value} value={tipo.value}>{tipo.label}</option>)}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                            <Col md={4}>
                    ...
                    {/* SECCIÓN 2: PRODUCTO */}
                    <div className="border rounded-3 p-3 mb-4" style={{ backgroundColor: '#f8f9fa' }}>
                        <h6 className="fw-bold text-secondary mb-3" style={{ fontSize: '0.85rem' }}><i className="bi bi-phone me-2"></i>Datos del Producto</h6>
                        <Row className="g-3">
                            <Col md={6}>
                                <Form.Group>
                            ...
                            <Col md={6}>
                                <Form.Group>
                                    <Form.Label className="small fw-semibold text-secondary">Valor del Producto ($) <span className="text-danger">*</span></Form.Label>
                                    <InputGroup className="rounded-3">
                                        <InputGroup.Text>$</InputGroup.Text>
                                        <Form.Control type="number" name="valor" value={formData.producto.valor} onChange={handleProductoChange}
                                            placeholder="0" className="rounded-end-3" disabled={isLoading} min={0} step={100} />
                                    </InputGroup>
                                </Form.Group>
                            </Col>
                        </Row>
                            ...
                            <Row className="g-3">
                                <Col md={4}>
                                    <Form.Group>
                                        ...
                                        <InputGroup className="rounded-3">
                                            <InputGroup.Text>$</InputGroup.Text>
                                            <Form.Control type="number" name="montoCuota" value={formData.montoCuota} onChange={handleChange}
                                                placeholder="0" className="rounded-end-3" disabled={isLoading} min={0} step={100} />
                                        </InputGroup>
                                    </Form.Group>
                                </Col>
                    ...
                    {mostrarEquipoCanje && (
                        <div className="border rounded-3 p-3 mb-4" style={{ backgroundColor: '#f8f9fa' }}>
                            ...
                            <Row className="g-3">
                                <Col md={6}>
                                    <Form.Group>
                                        <Form.Label className="small fw-semibold text-secondary">Nombre del Equipo <span className="text-danger">*</span></Form.Label>
                                        <Form.Control type="text" name="nombre" value={formData.equipoCanje.nombre} onChange={handleEquipoCanjeChange}
                                            placeholder="Ej: iPhone 12 Pro" className="rounded-3" disabled={isLoading} />
                                    </Form.Group>
                                </Col>
                                <Col md={6}>
                                    <Form.Group>
                                        <Form.Label className="small fw-semibold text-secondary">Modelo</Form.Label>
                                        <Form.Control type="text" name="modelo" value={formData.equipoCanje.modelo} onChange={handleEquipoCanjeChange}
                                            placeholder="Ej: A2408" className="rounded-3" disabled={isLoading} />
                                    </Form.Group>
                                </Col>
                                <Col md={4}>
                                    <Form.Group>
                                        <Form.Label className="small fw-semibold text-secondary">IMEI</Form.Label>
                                        <Form.Control type="text" name="imei" value={formData.equipoCanje.imei} onChange={handleEquipoCanjeChange}
                                            placeholder="15 dígitos" className="rounded-3" disabled={isLoading} maxLength={15} />
                                    </Form.Group>
                                </Col>
                                <Col md={4}>
                                    <Form.Group>
                                        <Form.Label className="small fw-semibold text-secondary">Color</Form.Label>
                                        <Form.Control type="text" name="color" value={formData.equipoCanje.color} onChange={handleEquipoCanjeChange}
                                            placeholder="Ej: Grafito" className="rounded-3" disabled={isLoading} />
                                    </Form.Group>
                                </Col>
                                <Col md={4}>
                                    <Form.Group>
                                        <Form.Label className="small fw-semibold text-secondary">Batería</Form.Label>
                                        <Form.Control type="text" name="bateria" value={formData.equipoCanje.bateria} onChange={handleEquipoCanjeChange}
                                            placeholder="Ej: 80%" className="rounded-3" disabled={isLoading} />
                                    </Form.Group>
                                </Col>
                                <Col md={6}>
                                ...
                                <Col md={6}>
                                    <Form.Group>
                                        ...
                                        <InputGroup className="rounded-3">
                                            <InputGroup.Text>$</InputGroup.Text>
                                            <Form.Control type="number" name="valorTasado" value={formData.equipoCanje.valorTasado} onChange={handleEquipoCanjeChange}
                                                placeholder="0" className="rounded-end-3" disabled={isLoading} min={0} step={100} />
                                        </InputGroup>
                                    </Form.Group>
                                </Col>
                    ...
                    {mostrarPagos && (
                        <div className="border rounded-3 p-3 mb-4" style={{ backgroundColor: '#f8f9fa' }}>
                                ...
                                <Row key={index} className="g-2 mb-2 align-items-end">
                                    <Col md={4}>
                                        <Form.Group>
                                            ...
                                            <InputGroup className="rounded-3">
                                                <InputGroup.Text>$</InputGroup.Text>
                                                <Form.Control type="number" value={pago.monto}
                                    ...
                                    <Col md={5}>
                                        <Form.Group>
                                            <Form.Label className="small fw-semibold text-secondary">Método de Pago</Form.Label>
                                            <Form.Select value={pago.metodo} onChange={(e) => handlePagoChange(index, 'metodo', e.target.value)} className="rounded-3" disabled={isLoading}>
                                                {metodosPago.map(metodo => <option key={metodo.value} value={metodo.value}>{metodo.label}</option>)}
                                            </Form.Select>
                                        </Form.Group>
                                    </Col>
                                    <Col md={3} className="d-flex gap-2">
                                        {index === 0 ? (
                                            <Button variant="outline-primary" onClick={agregarPago} className="rounded-3 flex-grow-1" disabled={isLoading} style={{ height: '38px' }}><i className="bi bi-plus-lg"></i></Button>
                                        ) : (
                                            <Button variant="outline-danger" onClick={() => eliminarPago(index)} className="rounded-3 flex-grow-1" disabled={isLoading} style={{ height: '38px' }}><i className="bi bi-trash3"></i></Button>
                                        )}
                                    </Col>
                                </Row>
                            ))}
                            <div className="mt-3 p-2 bg-light rounded-3">
                                <Row className="text-center">
                                    <Col><small className="text-muted d-block">Total Pagado</small><strong className="text-success">${calcularTotalPagos().toLocaleString()}</strong></Col>
                                    <Col><small className="text-muted d-block">Saldo Pendiente</small><strong className={getSaldoPendiente() > 0 ? 'text-danger' : 'text-success'}>${getSaldoPendiente().toLocaleString()}</strong></Col>
                                    <Col><small className="text-muted d-block">Total Venta</small><strong>${calcularMontoTotal().toLocaleString()}</strong></Col>
                                </Row>
                            </div>
                        </div>
                    )}
                                ...
                                <Row className="g-3">
                                    <Col md={6}>
                                        <Form.Group>
                                            ...
                                            <Form.Control type="text" name="nombre" value={formData.garante.nombre} onChange={handleGaranteChange}
                                                placeholder="Nombre del garante" className="rounded-3" disabled={isLoading} />
                                        </Form.Group>
                                    </Col>
                                    <Col md={6}>
                                        <Form.Group>
                                            <Form.Label className="small fw-semibold text-secondary">Apellido <span className="text-danger">*</span></Form.Label>
                                            <Form.Control type="text" name="apellido" value={formData.garante.apellido} onChange={handleGaranteChange}
                                                placeholder="Apellido del garante" className="rounded-3" disabled={isLoading} />
                                        </Form.Group>
                                    </Col>
                                    <Col md={4}>
                                        <Form.Group>
                                            <Form.Label className="small fw-semibold text-secondary">DNI <span className="text-danger">*</span></Form.Label>
                                            <Form.Control type="text" name="dni" value={formData.garante.dni} onChange={handleGaranteChange}
                                                placeholder="8 dígitos" className="rounded-3" disabled={isLoading} maxLength={8} />
                                        </Form.Group>
                                    </Col>
                                    <Col md={4}>
                                        <Form.Group>
                                            <Form.Label className="small fw-semibold text-secondary">CUIL</Form.Label>
                                            <Form.Control type="text" name="cuil" value={formData.garante.cuil} onChange={handleGaranteChange}
                                                placeholder="XX-XXXXXXXX-X" className="rounded-3" disabled={isLoading} />
                                        </Form.Group>
                                    </Col>
                                    <Col md={4}>
                                        <Form.Group>
                                            <Form.Label className="small fw-semibold text-secondary">Teléfono <span className="text-danger">*</span></Form.Label>
                                            <Form.Control type="text" name="telefono" value={formData.garante.telefono} onChange={handleGaranteChange}
                                    ...
                                    <Col md={6}>
                                        <Form.Group>
                                            ...
                                            <Form.Control type="email" name="email" value={formData.garante.email} onChange={handleGaranteChange}
                                                placeholder="ejemplo@correo.com" className="rounded-3" disabled={isLoading} />
                                        </Form.Group>
                                    </Col>
                                    <Col md={12}>
                                        <Form.Group>
                    ...
                    {/* RESUMEN */}
                    <div className="border rounded-3 p-3 mb-4" style={{ backgroundColor: '#f8f9fa' }}>
                        <h6 className="fw-bold text-primary mb-3" style={{ fontSize: '0.85rem' }}><i className="bi bi-clipboard-data me-2"></i>Resumen de la Venta</h6>
                        <Row className="g-3">
                            <Col md={6}><div className="p-2 bg-white rounded-3"><small className="text-muted d-block">Cliente</small><span className="fw-semibold">{clienteData?.nombreCompleto || clienteData?.nombre + ' ' + clienteData?.apellido}</span></div></Col>
                            <Col md={6}><div className="p-2 bg-white rounded-3"><small className="text-muted d-block">DNI</small><span className="fw-semibold">{clienteData?.dni}</span></div></Col>
                            <Col md={4}><div className="p-2 bg-white rounded-3"><small className="text-muted d-block">Tipo de Venta</small><span className="fw-semibold">{tiposVenta.find(t => t.value === formData.tipoVenta)?.label || formData.tipoVenta}</span></div></Col>
                    ...
                    <div className="d-flex gap-2 justify-content-end pt-3 mt-3 border-top">
                        ...
                        <Button variant="primary" type="submit" className="rounded-3 px-4"
                            style={{ backgroundColor: '#3483FA', borderColor: '#3483FA', fontWeight: '500', minWidth: '180px' }} disabled={isLoading}>
                            {isLoading ? <><Spinner as="span" animation="border" size="sm" className="me-2" />Procesando...</> : <><i className="bi bi-check-circle me-2"></i>Confirmar Venta</>}
                        </Button>
                    </div>
                </Form>
            </Card.Body>
