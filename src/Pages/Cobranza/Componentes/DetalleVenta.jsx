...
export const DetalleVenta = ({ 
  ...
  const handleAbrirEditarFecha = (cuota) => {
    setCuotaSeleccionada(cuota);
    setShowEditarFecha(true);
  };

  const handleEditarFechaExitoso = () => {
  ...
  const handleAbrirEditarMonto = (cuota) => {
    setCuotaSeleccionada(cuota);
    setShowEditarMonto(true);
  };

  const handleEditarMontoExitoso = () => {
    setShowEditarMonto(false);
    setCuotaSeleccionada(null);
    cargarDetalle();
    if (onRefresh) onRefresh();
  };

  const handleAbrirCambiarEstado = (cuota) => {
    setCuotaSeleccionada(cuota);
    setShowCambiarEstado(true);
  };

  const handleCambiarEstadoExitoso = () => {
  ...
  const handleAbrirAgregarNota = (cuota) => {
    setCuotaSeleccionada(cuota);
    setShowAgregarNota(true);
  };

  const handleAgregarNotaExitoso = () => {
    setShowAgregarNota(false);
    setCuotaSeleccionada(null);
    cargarDetalle();
    if (onRefresh) onRefresh();
  };
  ...
  return (
    <>
      ...
      >
        <Modal.Header closeButton className="border-0 pb-0">
          <Modal.Title className="fw-bold">
            <i className="bi bi-file-text me-2" style={{ color: '#3483FA' }}></i>
            Detalle de Venta
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body className="pt-3">
          {loading ? (
            <div className="text-center py-5">
              ...
              <p className="text-muted">Cargando detalle de la venta...</p>
            </div>
          ) : error ? (
            <Alert variant="danger" className="rounded-3">
              <div className="d-flex align-items-center">
                ...
                <div>
                  <h6 className="fw-bold mb-1">Error al cargar el detalle</h6>
            ...
            <>
              ...
              <Row className="g-3 mb-4">
                <Col md={6}>
                  <Card className="border-0 bg-light" style={{ borderRadius: '12px' }}>
                    <Card.Body className="p-3">
                      <h6 className="fw-bold text-primary mb-2" style={{ fontSize: '0.85rem' }}>
                ...
                <Col md={6}>
                  <Card className="border-0 bg-light" style={{ borderRadius: '12px' }}>
                    <Card.Body className="p-3">
                      <h6 className="fw-bold text-secondary mb-2" style={{ fontSize: '0.85rem' }}>
                  ...
                  <Card className="border-0 shadow-sm text-center" style={{ borderRadius: '12px' }}>
                    <Card.Body className="py-3">
                      ...
                      <h5 className="fw-bold mb-0 text-danger">
                        {formatMonto(venta.resumen?.montoPendiente)}
                      </h5>
                    </Card.Body>
                  </Card>
                </Col>
              ...
              {/* ==========================================
                  SECCIÓN 3: PROGRESO DE PAGO
                  ========================================== */}
              <div className="mb-4">
                <div className="d-flex justify-content-between align-items-center mb-1">
                  <small className="text-muted">Progreso de pago</small>
                  <small className="fw-semibold">
                    {venta.resumen?.cuotasPagadas || 0}/{venta.resumen?.totalCuotas || 0} cuotas
                  </small>
                </div>
                <ProgressBar
                  now={((venta.montoPagado || 0) / (venta.montoTotal || 1)) * 100}
                  variant={venta.montoPagado >= venta.montoTotal ? 'success' : 'primary'}
              ...
              <div className="mb-3">
                <h6 className="fw-bold mb-3" style={{ fontSize: '0.95rem' }}>
                  <i className="bi bi-grid me-2" style={{ color: '#3483FA' }}></i>
                  Cuotas
                    ...
                    <tbody>
                      {venta.cuotas?.map((cuota) => (
                        <tr key={cuota.numeroCuota}>
                          ...
                          <td>
                            {cuota.estado_cuota === 'pendiente' && cuota.diasAtraso > 0 ? (
                          ...
                          <td>
                            <div className="d-flex gap-1 justify-content-center flex-wrap">
                              ...
                              {/* Botón Editar Fecha - solo si no está pagada */}
                              {cuota.estado_cuota !== 'pagada' && (
                                <Button
                                  variant="outline-primary"
                                  size="sm"
                                  className="rounded-3"
                                  onClick={() => handleAbrirEditarFecha(cuota)}
                              ...
                              {/* Botón Editar Monto - solo si no está pagada */}
                              {cuota.estado_cuota !== 'pagada' && (
                                <Button
                                  variant="outline-warning"
                                  size="sm"
                                  className="rounded-3"
                                  onClick={() => handleAbrirEditarMonto(cuota)}
                                  title="Editar monto"
                                >
                              ...
                              {/* Botón Cambiar Estado - solo si no está pagada */}
                              {cuota.estado_cuota !== 'pagada' && (
                                <Button
                                  variant="outline-secondary"
                                  size="sm"
                                  className="rounded-3"
                                  onClick={() => handleAbrirCambiarEstado(cuota)}
                              ...
                              {/* Botón Agregar Nota - siempre disponible */}
                              <Button
                                variant="outline-info"
                                size="sm"
                                className="rounded-3"
                                onClick={() => handleAbrirAgregarNota(cuota)}
                                title="Agregar nota"
                              >
                ...
                <div className="mt-3">
                  ...
                  <div className="bg-light rounded-3 p-3">
                    {venta.notas.map((nota, index) => (
                      <div key={index} className="d-flex justify-content-between align-items-center border-bottom py-1 last:border-0">
                        <span className="small">{nota.texto}</span>
                        <span className="text-muted small">
                          {formatFecha(nota.fecha)}
                          {nota.usuario?.nombre && ` - ${nota.usuario.nombre}`}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="mt-3 text-muted small">
                <Row>
                  <Col md={6}>
                    ...
                    Venta realizada: {formatFechaCompleta(venta.fechaRealizada)}
                  </Col>
                  <Col md={6}>
        ...
        <style>{`
          ...
          .detalle-modal .modal-body::-webkit-scrollbar-track {
            background: #f1f1f1;
            border-radius: 3px;
          }
          ...
          .detalle-modal .modal-body::-webkit-scrollbar-thumb:hover {
            background: #a8a8a8;
          }
          ...
          .cuotas-table-wrapper::-webkit-scrollbar {
            width: 6px;
          }

          .cuotas-table-wrapper::-webkit-scrollbar-track {
            background: #f1f1f1;
            border-radius: 3px;
          }

          .cuotas-table-wrapper::-webkit-scrollbar-thumb {
            background: #c1c1c1;
            border-radius: 3px;
          }

          .cuotas-table-wrapper::-webkit-scrollbar-thumb:hover {
            background: #a8a8a8;
          }

          .detalle-modal .table th {
            position: sticky;
            top: 0;
            background: #f8f9fa;
            z-index: 1;
      ...
      {/* Modal Editar Fecha */}
      <ModalEditarFechaCuota
        show={showEditarFecha}
        onHide={() => {
          setShowEditarFecha(false);
          setCuotaSeleccionada(null);
        }}
        ...
        onSuccess={handleEditarFechaExitoso}
        usuario={usuario}
      />

      {/* Modal Editar Monto */}
      <ModalEditarMontoCuota
        show={showEditarMonto}
        onHide={() => {
          setShowEditarMonto(false);
          setCuotaSeleccionada(null);
        }}
        venta={venta}
        cuota={cuotaSeleccionada}
        onSuccess={handleEditarMontoExitoso}
        usuario={usuario}
      />

      {/* Modal Cambiar Estado */}
      <ModalCambiarEstadoCuota
        ...
        venta={venta}
        cuota={cuotaSeleccionada}
        onSuccess={handleCambiarEstadoExitoso}
        usuario={usuario}
      />

      {/* Modal Agregar Nota */}
      <ModalAgregarNotaCuota
        show={showAgregarNota}
        onHide={() => {
          setShowAgregarNota(false);
          setCuotaSeleccionada(null);
        }}
        venta={venta}
        cuota={cuotaSeleccionada}
        onSuccess={handleAgregarNotaExitoso}
        usuario={usuario}
      />
    </>
  );
